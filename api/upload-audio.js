...
module.exports = async (req, res) => {
  ...
  try {
    ...
    try {
      await dbConnect();
      console.log('Connected to MongoDB in audio upload handler');
    } catch (dbError) {
    ...
    form.parse(req, async (err, fields, files) => {
      ...
      try {
        ...
        const newFilename = `recording_${timestamp.replace(/:/g, '-')}.webm`;
        ...
        try {
          ...
          // Upload to S3 - store in 'audio' folder
          fileUrl = await uploadToS3(fileContent, newFilename, 'audio/webm', 'audio');
          ...
          return res.status(500).json({ error: 'Failed to upload audio to S3' });
        }
        ...
        if (global.db) {
          ...
          const mediaRecord = new Media({
            ...
            metadata: {
              ...
              duration: fields.duration || 'unknown',
          ...
          console.log('Audio metadata saved to MongoDB');
        }
